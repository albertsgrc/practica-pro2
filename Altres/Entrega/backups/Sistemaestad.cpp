...
void Sistema::actualitza_espais(Lpair&l, int k, Lpair::iterator it) {
...
Sistema::Sistema(int pobmax) {
    ...
    espais = vector<Lpair>(pobmax - 2); // Por la misma razón que en la matriz no hace falta tener la información de los dos últimos organismos
}
...
void Sistema::aplicar_ronda_reproduccion(Ranking&rank) {
    ...
    while (i < pobaux - 1 and pob < vorg.size()) {
        if (not agafat[i] and not vorg[i].esta_muerto()) { // Lo tratamos sólo si no se ha emparejado aún en la ronda
            Lpair::iterator it = espais[i].begin();
            int j = i + 1; // Empezamos a buscar desde el siguiente identificador
            bool nollista = it == espais[i].end();
            if (not nollista and (*it).first == j) {
                j += (*it).second - (*it).first + 1;
                //cout << "****" << (*it).second - (*it).first << endl;
                suma += (*it).second - (*it).first;
                total += (*it).second - (*it).first + 1;
                ++it;
                nollista = it == espais[i].end();
            }
            while (j < pobaux and (emparejados[i][j - i - 1] or agafat[j] or vorg[j].esta_muerto())) {
                if (not nollista and (*it).first == j) {
                    j += (*it).second - (*it).first + 1;
                    suma += (*it).second - (*it).first;
                    total += (*it).second - (*it).first + 1;
            ...
            if (j < pobaux) { // Si se ha encontrado, emparejamos
                actualitza_espais(espais[i], j, it);
                //cout << "Organismo " << i+1 << " : ";
                //escribir_lista(espais[i]);
