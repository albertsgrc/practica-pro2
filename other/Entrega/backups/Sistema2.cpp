...
void Sistema::emparejar_organismos(Vbool&agafat, Ranking&rank, int i, int j) {
...
void Sistema::aplicar_ronda_reproduccion(Ranking&rank) {
    ...
    while (i < pobaux - 1 and pob < vorg.size()) {
        if (not agafat[i] and not vorg[i].esta_muerto()) { // Lo tratamos sólo si no se ha emparejado aún en la ronda
            while (ini_emp[i] < pobaux and (vorg[ini_emp[i]].esta_muerto() or emparejados[i][ini_emp[i] - i - 1])) ++ini_emp[i];
            if (ini_emp[i] < pobaux) {
                if (not agafat[ini_emp[i]]) emparejar_organismos(agafat, rank, i, ini_emp[i]);
                else {
                    int j = ini_emp[i] + 1; // Empezamos a buscar desde el siguiente identificador
                    while (j < pobaux and (emparejados[i][j - i - 1] or agafat[j] or vorg[j].esta_muerto())) ++j; // Buscamos la pareja
                    if (j < pobaux) emparejar_organismos(agafat, rank, i, j);
                }
            }
        }
        ++i;
